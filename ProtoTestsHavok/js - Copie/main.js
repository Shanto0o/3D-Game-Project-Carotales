...
function createMovingPlatform(scene, p_from, p_to, speed = 2) {
  BABYLON.SceneLoader.ImportMesh(null, "images/", "plat.glb", scene, (meshes) => {
    if (!meshes || meshes.length === 0) {
      console.error("Aucun mesh importé depuis plat.glb.");
      return;
    }

    // Filtrer les meshes visibles avec géométrie
    const meshList = meshes.filter(m => m instanceof BABYLON.Mesh && m.geometry);

    if (meshList.length === 0) {
      console.error("Aucun mesh géométrique trouvé à fusionner.");
      return;
    }

    // Fusionner tous les meshes
    const merged = BABYLON.Mesh.MergeMeshes(meshList, true, true, undefined, false, true);

    if (!merged) {
      console.error("Échec de la fusion des meshes.");
      return;
    }

    merged.name = "movingPlatform";
    merged.position = p_from.clone();

    const platformAggregate = new BABYLON.PhysicsAggregate(
      merged,
      BABYLON.PhysicsShapeType.MESH,
      { mass: 0, restitution: 0.3, friction: 10 },
      scene
    );

    platformAggregate.body.setMotionType(BABYLON.PhysicsMotionType.ANIMATED);

    let currentPos = p_from.clone();
    let direction = p_to.subtract(p_from).normalize();
    let goingTo = true;
    let previousPos = p_from.clone();

    scene.onBeforeRenderObservable.add(() => {
      const deltaTime = scene.getEngine().getDeltaTime() * 0.001;
      const moveStep = direction.scale(speed * deltaTime);

      currentPos.addInPlace(moveStep);

      const target = goingTo ? p_to : p_from;
      if (BABYLON.Vector3.Distance(currentPos, target) < speed * deltaTime) {
        currentPos = target.clone();
        goingTo = !goingTo;
        direction = (goingTo ? p_to : p_from).subtract(currentPos).normalize();
      }

      const velocity = currentPos.subtract(previousPos).scale(1 / deltaTime);
      previousPos.copyFrom(currentPos);

      merged.physicsBody.setTargetTransform(currentPos, BABYLON.Quaternion.Identity(), velocity);
    });
  },
  null,
  (scene, message, exception) => {
    console.error("Erreur de chargement de plat.glb:", message, exception);
  });
...
function createGround(scene, level) {
  ...
  if (level === 1) {
      BABYLON.SceneLoader.ImportMesh("", "images/", "niveau1.glb", scene, function (meshes) {
          ...
          createMovingPlatform(scene, p1_from, p1_to,/* speed */ 4);
          const p2_from = new BABYLON.Vector3(5, 2, 0);
          const p2_to   = new BABYLON.Vector3(-5, 5, 10);
          createMovingPlatform(scene, p2_from, p2_to,/* speed */ 4);
