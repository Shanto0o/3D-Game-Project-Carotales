...
export default class Enemy {
  ...
  constructor(scene, path, speed = 0.05, detectionRange = 10, audioManager, ) {
    ...
    console.log("Enemy created with audioManager:", this.am);
  ...
  update(player) {
    ...
    const bg = this.scene.getSoundByName("BackgroundMusic");  
         ...
         // ——————————————————————————————————————
     // 2) Jouer / arrêter les sons sur transition
     // ——————————————————————————————————————
    if (this.am && this.state !== oldState) {
      // On vient d’entrer en PATROL
    ...
    if (angry) angry.stop();
     if (bg && !bg.isPlaying) { bg.play(); }
      ...
      // On vient d’entrer en CHASE
      else if (this.state === 'CHASE') {
         if (bg && bg.isPlaying) { bg.pause(); }

    this.am.play('trigger');

// 2) on ne lance “angry” que s’il n’est pas déjà en cours
const angry = this.am.sounds.get('angry');
if (angry && !angry.isPlaying) {
  angry.play();
}
  
  }
      // On sort de CHASE
      if (oldState === 'CHASE' && this.state !== 'CHASE') {
          const angry = this.am.sounds.get('angry');
if (angry && angry.isPlaying) {
  angry.stop();
}
