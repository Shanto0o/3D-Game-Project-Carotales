...
export default class Player {
  ...
  move(inputStates, camera) {
    ...
    if (inputStates.down)  {this.inputDirection.x += Math.sin(Math.PI/2 - camera.alpha); this. inputDirection.z += Math.cos(Math.PI/2 - camera.alpha);}
    if (inputStates.left)  {this.inputDirection.x += Math.sin(3*Math.PI - camera.alpha); this. inputDirection.z += Math.cos(3*Math.PI - camera.alpha);}
    if (inputStates.right) {this.inputDirection.x += Math.sin(Math.PI*2 - camera.alpha); this. inputDirection.z += Math.cos(Math.PI*2 - camera.alpha);}
    if (inputStates.jump)  this.wantJump ++;
    

    // 3) Orientation du joueur si on bouge
    if (this.inputDirection.lengthSquared() > 0.0001) {
